...
class MinCoreUsage():
    def min_core_usage(self, assigned, cores, tasks_instances, prob):
        ...
        # 2.9 If a task instance uses a core, the core is marked used. (Generated by ChatGPT) (From C1111 - C1144)
        for core in cores:
            for task in tasks_instances:
                for instance in task['value']:
                    prob += assigned[(task['name'], instance['instance'], core['name'])] <= unused[(core['name'])]
